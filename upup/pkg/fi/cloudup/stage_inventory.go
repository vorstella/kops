...
const (
	...
	AssetCompressedFile = "compressed-file"
)
...
type CompressedFileAssetTransferer struct {
	fileRepo string
}
...
func NewStageInventory(fileRepo string, stageFiles bool, containerRepo string, stageContainers bool, assets *api.Inventory) *StageInventory {
	...
	if stageFiles {
		assetTransferers[AssetCompressedFile] = CompressedFileAssetTransferer{
			fileRepo: fileRepo,
		}
...
func (f FileAssetTransferer) Transfer(i interface{}, t string) error {
	...
	if err := transferFile(asset.Location, f.fileRepo); err != nil {
		return err
	}

	if asset.SHA != "" {
		return transferFile(asset.SHA, f.fileRepo)
	}
...
func (c CompressedFileAssetTransferer) Transfer(i interface{}, t string) error {

	asset := i.(api.CompressedFileAsset)
	glog.Infof("File asset transfer: %s - %s\n", t, asset.Location)

	if err := transferFile(asset.Location, c.fileRepo); err != nil {
		return err
	}

	if asset.SHA != "" {
		return transferFile(asset.SHA, c.fileRepo)
	}
...
func transferFile(location string, fileRepo string) error {
	...
	s3Path := fileRepo + fileURL.Path
	glog.Infof("FileAssetTransferer.Transfer: s3Path: %s\n", s3Path)
	destinationRegistry, err := vfs.Context.BuildVfsPath(s3Path)
	if err != nil {
		return fmt.Errorf("Error FileAssetTransferer.Transfer parsing registry path %q: %v", fileRepo, err)
	}

	glog.Infoln("FileAssetTransferer.Transfer: writing data...")
	if err = destinationRegistry.WriteFile(data); err != nil {
		return fmt.Errorf("Error FileAssetTransferer.Transfer destination path %q: %v", fileRepo, err)
	}
